...
describe("schema", function()
  ...
  it("allows policy overrides for reject and telemetry settings", function()
    ...
    assert.equals("number", override_fields.emit_sample_rate.type)
    assert.same({ 0, 1 }, override_fields.emit_sample_rate.between)
    assert.equals("boolean", override_fields.emit_include_versions.type)
    assert.same({ "logfmt", "json" }, override_fields.emit_format.one_of)
  end)
